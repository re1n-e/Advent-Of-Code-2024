    ...
    if let Ok(lines) = read_lines(path) {
        for line in lines {
            if let Ok(numbers) = line {
                ...
                if is_safe_report(&levels) || can_be_safe_by_removing_one(&levels) {
                    safe_reports += 1;
                }
            }
        }
...
fn is_safe_report(levels: &[i32]) -> bool {
    ...
    for pair in levels.windows(2) {
        ...
        if diff.abs() < 1 || diff.abs() > 3 || diff == 0 {
            return false;
        }
...
fn can_be_safe_by_removing_one(levels: &[i32]) -> bool {
    let len = levels.len();

    for i in 0..len {
        let left = &levels[..i];
        let right = &levels[i + 1..];
        if is_safe_report(&merge_slices(left, right)) {
            return true; 
        }
    }
    false
...
fn merge_slices<'a>(left: &'a [i32], right: &'a [i32]) -> Vec<i32> {
